...
class DeviceMonitoringConfig(AppConfig):
    ...
    def is_working_changed_receiver(
        ...
        # to reboot caused by firmware upgrade, don't trigger checks and avoid notifications
        if 'Unable to connect' in failure_reason:
            device_monitoring.disabled_checks = True
        ...
        if status == initial_status == 'ok':
            device_monitoring.disabled_checks = False
            device_monitoring.save()
            return
